 ...
 * cycle TMs, session set weights + estimated 1RMs, bodyweight entries,
 * assistance/supporting-lift logged weights) from the profile's current
 * unit to `to`. No-ops when there is no profile, or the profile is already
 * in `to`. Runs as a single Dexie transaction so a failure partway through
...
export async function convertUnits(to: Unit): Promise<void> {
  ...
  await db.transaction(
    'rw',
    [db.profile, db.lifts, db.cycles, db.sessions, db.bodyweight, db.assistance, db.supportingDone],
    async () => {
      ...
      const assistance = await db.assistance.toArray();
      await db.assistance.bulkPut(
        assistance.map((e) => ({
          ...e,
          weight: e.weight == null ? e.weight : roundToIncrement(e.weight * factor, target),
        })),
      );

      const supportingDone = await db.supportingDone.toArray();
      await db.supportingDone.bulkPut(
        supportingDone.map((s) => ({
          ...s,
          weight: s.weight == null ? s.weight : roundToIncrement(s.weight * factor, target),
        })),
      );
    },
  );
