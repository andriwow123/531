...
- States:
  ...
  - **Left running (forgotten):** once the timer has run for **more than 3 hours** (or started on an earlier day), the running display is replaced by an emphasized prompt — "Still running since 6:04 PM — forgot to end it?" — with **"Set finish time"** (primary) and "End now".
...
- WorkoutTimer: Start → running; End → duration; Edit times → saved (incl. past-midnight); Reset clears. "Forgot to end it?" → set a finish time → ended with that duration; prefill = session-saved time when available, else start + 1 h, capped at now; a future or before-start finish is rejected with the inline error; a timer running > 3 h (or from an earlier day) shows the forgotten prompt. History shows duration on a matching entry.
