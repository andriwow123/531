...
- States:
  ...
  - **Left running (forgotten):** once the timer has run for **more than 3 hours**, the running display is replaced by an emphasized prompt — "Still running since 6:04 PM — forgot to end it?" — with **"Set finish time"** (primary) and "End now".
