...
- States:
  ...
  - **Running:** "Workout · 12:34" ticking every second from `startedAt` (survives reload — it's stored) + "End workout".
...
- WorkoutTimer: Start → running; End → duration; Edit times → saved (incl. past-midnight); Reset clears. History shows duration on a matching entry.
