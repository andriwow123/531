...
- **Running** (`startedAt`, no `endedAt`), ticking via a 1 s `setInterval` (cleared on stop/unmount):
  ...
  - when `isLeftRunning(startedAt, now)`: an emphasized box (accent-soft background) `Still running since {start time in the device's own time format, e.g. 18:04} — forgot to end it?` with **"Set finish time"** (primary → finish mode) and **"End now"**.
